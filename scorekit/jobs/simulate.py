...
PERSONAS: dict[str, dict[tuple[str, str], float]] = {
    ...
    # The four below carry interaction effects as well (see COMBOS). Their single-tag
    # weights are deliberately mild, so most of what decides a like lives in the pairs and
    # a linear ranker has something it structurally cannot reach.
    "nocturnes-not-etudes": {
        ("composer", "chopin"): 0.8, ("era", "romantic"): 0.5,
    },
    "bach-fugues-only": {
        ("composer", "bach"): 0.8, ("era", "baroque"): 0.5,
    },
    "romantic-sonatas-classical-miniatures": {
        ("era", "romantic"): 0.5, ("era", "classical"): 0.5,
    },
    "tutorials-for-hard-pieces-only": {
        ("format", "tutorial"): 0.6, ("form", "etude"): 0.4,
    },
}

# Interaction effects: taste that a weighted sum over single tags cannot express.
#
# "Chopin nocturnes yes, Chopin etudes no" is not composer-weight plus form-weight. No
# setting of those two numbers produces it, because the etude penalty has to apply only in
# the presence of Chopin. Real taste is full of this, and it is the whole reason to reach
# for a tree model: a tree can branch on one tag and then ask about another, a linear sum
# cannot.
#
# Without any of these the synthetic data is generated by a weighted sum over tags, which
# is the exact functional form of the hand-tuned ranker, so the baseline is close to the
# true model and nothing can meaningfully beat it.
COMBOS: dict[str, list[tuple[tuple[str, str], tuple[str, str], float]]] = {
    "nocturnes-not-etudes": [
        (("composer", "chopin"), ("form", "nocturne"), 1.8),
        (("composer", "chopin"), ("form", "etude"), -2.0),
    ],
    "bach-fugues-only": [
        (("composer", "bach"), ("form", "fugue"), 1.8),
        (("composer", "bach"), ("form", "prelude"), -1.6),
    ],
    "romantic-sonatas-classical-miniatures": [
        (("era", "romantic"), ("form", "sonata"), 1.6),
        (("era", "classical"), ("form", "sonata"), -1.6),
        (("era", "classical"), ("form", "minuet"), 1.4),
    ],
    "tutorials-for-hard-pieces-only": [
        (("format", "tutorial"), ("form", "etude"), 1.8),
        (("format", "tutorial"), ("form", "minuet"), -1.6),
    ],
}
...
def taste(tags: list[tuple[str, str]], persona: dict[tuple[str, str], float],
          combos: list[tuple[tuple[str, str], tuple[str, str], float]] | None = None) -> float:
    """How much this persona wants the piece: single-tag weights plus any pair effects."""
    present = set(tags)
    score = sum(persona.get(t, 0.0) for t in present)
    for a, b, w in (combos or []):
        if a in present and b in present:
            score += w
    return score


def _p_like(tags: list[tuple[str, str]], persona: dict[tuple[str, str], float],
            combos: list[tuple[tuple[str, str], tuple[str, str], float]] | None = None) -> float:
    return 1.0 / (1.0 + math.exp(-(_BIAS + _SCALE * taste(tags, persona, combos))))
    ...
    for name, persona in PERSONAS.items():
        ...
        for session in range(sessions):
            ...
            for position, card in enumerate(batch):
                ...
                p = _p_like(piece_tags, persona, COMBOS.get(name))
                roll = rng.random()
