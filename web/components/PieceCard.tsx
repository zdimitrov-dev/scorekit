  ...
  return (
    ...
    >
      {card.thumbnail_url ? (
        ...
        <div
          style={{ height: phHeight(card.id) }}
          className="flex w-full items-end bg-gradient-to-br from-[var(--surface-2)] to-[#2b2b38] p-4"
        >
          <span className="line-clamp-2 font-mono text-xs text-[var(--muted)]">
            {card.piece?.composer ?? card.author ?? "score"}
          </span>
        </div>
      )}
